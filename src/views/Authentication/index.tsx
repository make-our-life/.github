...
import { SignInRequestDto, SignUpRequestDto } from '@src/apis/request/auth';
import { signInRequest, signUpRequest } from '@src/apis';
import { SignInResponseDto, SignUpResponseDto } from '@src/apis/response/auth';
...
export default function Authentication() {
  ...
  const SignInCard = () => {
    ...
    const signInResponse = (responseBody: SignInResponseDto | ResponseDto | null) => {
      if (!responseBody) {  // Backend 문제로 responseBody가 null 나올 수 있음
        alert('네트워크 에러');
        return;
      }
  ...
  const SignUpCard = () => {
    ...
    const signUpResponse = (responseBody: SignUpResponseDto | ResponseDto | null) => {
      ...
      if(code === 'DT') {
        ...
        setTelNumberErrorMessage('중복되는 핸드폰 번호호입니다.');
      }
 ...
 */
    ...
    const onNextButtonClickHandler = () => {
      // 이메일 정규식 패턴 : [a-zA-Z0-9] 몇개가 들어가든 상관 없음 + @ + [-.] 올 수 있고 [a-zA-Z0-9] 여러개 + . + [a-zA-Z0-9] 2~4개
      const emailPattern = /^[a-zA-Z0-9]*@([-.]?[a-zA-Z0-9])*\.[a-zA-Z]{2,4}$/;
      const isEmailPattern = emailPattern.test(email);
      if(!email) {// 이메일 존재하지 않으면 에러 메시지 발생
        setEmailError(true);
      ...
      if(!isCheckedPassword) {
        ...
        setPasswordErrorMessage('비밀번호 8자 시상 입력해주세요.');
      }
    ...
    const onSignUpButtonClickHandler = () => {
      ...
      const requestBody: SignUpRequestDto = {
        email, password, nickname, telNumber, agreedPersonal
      }
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            {page === 2 && (
              <>
                <InputBox ref={nicknameRef} label='닉네임*' type='text' placeholder='닉네임을 입러해주세요.' value={nickname} onChange={onNicknameChangeHandler} error={isNicknameError} message={nickNameErrorMessage} onKeyDown={onNicknameKeyDownHandler}/>
                <InputBox ref={telNumberRef} label='핸드폰 번호*' type='text' placeholder='전화번호를 입력해주세요.' value={telNumber} onChange={onTelNumberChangeHandler} error={isTelNumberError} message={telNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler}/>
              </>
            )}
